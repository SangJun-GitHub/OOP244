...
int main() 
{
   ...
   Kingdom* extra_Kingdom = nullptr;
   extra_Kingdom = new Kingdom[count + 1];

   // TODO: copy elements from original array into this newly allocated array
   for (int i = 0; i < count; i++)
   {
      extra_Kingdom[i] = pKingdom[i];
   }
   ...
   {
      cout << "Enter the name of the Kingdom" << ": ";
      cin >> pKingdom[j].m_name;
      cout << "Enter the number of people living in " << pKingdom[j].m_name << ": ";
      cin >> pKingdom[j].m_population;
   }
	count++;
	cout << "==========\n" << endl;
   ...
   delete[] extra_Kingdom;
   extra_Kingdom = nullptr;
	return 0;
}
